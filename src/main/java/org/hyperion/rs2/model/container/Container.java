...
public class Container {
	...
	/**
	 * The index of equipment slots.
	 */
	public static final int HELM = 0, CAPE = 1, AMULET = 2, WEAPON = 3, BODY = 4, SHIELD = 5,
	...
	/**
	 * The interface of this container.
	 */
	...
	/**
	 * Creates a container with the specified capacity and interface.
	 * @param player The player the container belongs to.
	 * @param type The type of this container.
	 * @param capacity The capacity of this container.
	 * @param containerInterface The interface of this container.
	 */
	public Container(Player player, Type type, int capacity, ContainerInterface containerInterface) {
		this.player = player;
		this.type = type;
		this.capacity = capacity;
		this.items = new Item[capacity];
	...
	/**
	 * Refreshes all the items in a container.
	 * Requires the container to be of the type which has a <code>ContainerInterface</code>.
	 */
	public void refresh() {
		if (containerInterface == null) {
			System.out.println("[WARNING] Container Interface is null!");
			return;
		}
		
		player.getActionSender().sendUpdateItems(containerInterface.getInterfaceId(), 
				containerInterface.getChildId(), containerInterface.getType(), items);
	}
	...
	/**
	 * Checks if the container contains the specified item, supports quantity checking.
	...
	public boolean hasItem(Item item) {	
		for (int i = 0; i < items.length; i++) {
			if (items[i] != null && items[i].getId() == item.getId()) {
				if (items[i].getCount() >= item.getCount()) {
					return true;
				}
			}
		}
